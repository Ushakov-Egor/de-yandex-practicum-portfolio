# DWH для нескольких источников. Система курьерской доставки

## Описание задачи

Сервис доставки нуждался в DWH для отслеживания эффективности курьеров и расчёта ежемесячной зарплаты. Сырые данные о курьерах и доставках получаются через внешний HTTP API и должны проходить через трёхслойную архитектуру (STG → DDS → CDM) для формирования витрины отчёта курьеров, используемой в расчёте выплат.

## Что было сделано

- Реализована интеграция данных курьерской службы (API) с данными подсистемы заказов, хранящимися в DWH, построенном в ходе предыдущих заданий курса
- Разработан DDL для всех трёх слоёв: STG (хранение сырого JSON), DDS (нормализованные таблицы измерений и фактов: `dm_couriers`, `dm_deliveries`, `dm_delivery_addresses`, `fct_courier_deliveries`), CDM (`dm_courier_report`)
- Реализованы Python-классы загрузчиков для каждого слоя: постраничное получение данных через API (offset/limit) для STG, инкрементальный upsert для DDS, агрегация в CDM
- Созданы три независимых Airflow DAG с расписанием каждые 15 минут: загрузчик STG, трансформатор DDS, агрегатор CDM
- Витрина CDM рассчитывает выплаты курьерам на основе количества доставок, среднего рейтинга и ступенчатых правил комиссии за календарный месяц

## Стек и инструменты

`Apache Airflow` `PostgreSQL` `Python` `SQL` `REST API` `DDL` `ETL`

## Структура папки

- `src/` — весь исходный код
  - `dags/` — Airflow DAG и классы загрузчиков по слоям (stg/, dds/, cdm/)
  - `sql_scripts/` — SQL-скрипты по слоям: `stg/ddl/`, `dds/ddl/`, `dds/dml/`, `cdm/ddl/`, `cdm/dml/`
- `api_entities.md` — спецификация API и описание слоёв DWH
...
Путь к SQL-файлу строится через `Path(__file__).resolve().parents[N] / 'sql_scripts' / '<layer>' / '<ddl|dml>'`, где `N` зависит от глубины вложенности файла. SQL выполняется в одной транзакции, после чего в `dds.srv_wf_settings` сохраняется checkpoint с временем последнего запуска.
